        ...
        <div className="flex flex-wrap gap-1 px-4 pb-3">
          {CATS.map(([id, label]) => (
            <button key={id} onClick={() => { setCat(id); setQ(''); }} data-testid={`asset-cat-${id}`}
                    className={`shrink-0 px-2.5 py-1 rounded-lg text-[11.5px] font-semibold whitespace-nowrap border transition-colors ${cat === id && !q ? 'bg-[#14b877] text-[#03150d] border-[#14b877]' : 'bg-white/[0.07] text-white/80 border-white/15 hover:text-white hover:bg-white/[0.12] hover:border-white/25'}`}>
              {label}
            </button>
          ))}
        </div>
